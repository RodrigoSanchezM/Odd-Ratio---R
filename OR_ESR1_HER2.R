#Script para el cálculo del Odds Ratio entre presencia del alelo mt intergen y grupos sensibles y no sensibles a HER2
#Julio 2025 
...
table(tabla_pacientes$`Genotipo ESR1`,tabla_pacientes$HER2)
...
table2x2_ESR1 <- table(tabla_pacientes$`Presencia Alelo ESR1`, tabla_pacientes$HER2)
...
#Cálculo OR crudo
OR_ESR1_HER2 <- epi.2by2(table2x2_ESR1, method = "case.control",  conf.level = 0.95)



#Selección de columnas y formación de una nueva tabla
OR_ajustado_ESR1_HER2 <- tabla_pacientes %>% select("HER2","Edad","Presencia Alelo ESR1")


#Reemplazo valores columna "GRUPO". Positivo = 1 , Negativo = 0
OR_ajustado_ESR1_HER2 <- OR_ajustado_ESR1_HER2 %>% 
...
OR_ajustado_ESR1_HER2$HER2 <- as.factor(OR_ajustado_ESR1_HER2$HER2)

#Establecer la variable EDAD como variable NUMÉRICA y no categórica.
OR_ajustado_ESR1_HER2$Edad <- as.numeric(OR_ajustado_ESR1_HER2$Edad)




#Cálculo de la regresión logística 
OR_ajustado_ESR1_HER2 <- glm (HER2 ~ `Presencia Alelo ESR1`+ Edad, data = OR_ajustado_ESR1_HER2, family = binomial)

#Selección de FILAS COEF
summary_ORtab_ESR1_HER2 <- summary (OR_ajustado_ESR1_HER2)$coef



OR_ESR1_HER2 <- data.frame(variable = rownames(summary_ORtab_ESR1_HER2),
                         oddsratio = round(exp(summary_ORtab_ESR1_HER2[,1]),3),
                         ci_low = round(exp(summary_ORtab_ESR1_HER2[,1] - 1.96*summary_ORtab_ESR1_HER2[,2]),3),
                         ci_high = round(exp(summary_ORtab_ESR1_HER2[,1] + 1.96*summary_ORtab_ESR1_HER2[,2]),3),
                         pval = scales::pvalue(summary_ORtab_ESR1_HER2[,4]),
                         row.names = NULL)
