...
table2x2_intergen <- table(tabla_pacientes$`Genotipo Intergen`, tabla_pacientes$ER)


#Renombramiento de las filas ("CC", "CT + TT) en reemplazo de los nombres "TRUE" y "FALSE"
rownames(table2x2_intergen) <- c("CC", "CT + TT")
...
table2x2_intergen <- table2x2_intergen[nrow(table2x2_intergen):1,]


#reordenamiento del orden de las columnas
table2x2_intergen <- table2x2_intergen[, c("Positivo", "Negativo")]



#Cálculo OR crudo
OR_intergen_ER <- epi.2by2(table2x2_intergen, method = "case.control",  conf.level = 0.95)




#Selección de columnas y formación de una nueva tabla
OR_ajustado_intergen_ER <- tabla_pacientes %>% select("ER","Edad","Presencia Alelo Intergen")


#Reemplazo valores columna "GRUPO"
OR_ajustado_intergen_ER <- OR_ajustado_intergen_ER %>% 
...
#que la variable dependiente sea un factor con dos niveles (por ejemplo, "Control" vs "Cancer").
  ...
  OR_ajustado_intergen_ER$ER <- as.factor(OR_ajustado_intergen_ER$ER)

#Establecer la variable EDAD como variable NUMÉRICA y no categórica.
  OR_ajustado_intergen_ER$Edad <- as.numeric(OR_ajustado_intergen_ER$Edad)

  
  
#Cálculo de la regresión logística 
OR_ajustado_intergen_ER <- glm (ER ~ `Presencia Alelo Intergen`+ Edad, data = OR_ajustado_intergen_ER, family = binomial)
  
#Selección de FILAS COEF
summary_ORtab_Intergen_ER <- summary (OR_ajustado_intergen_ER)$coef



OR_Intergen_ER <- data.frame(variable = rownames(summary_ORtab_Intergen_ER),
           oddsratio = round(exp(summary_ORtab_Intergen_ER[,1]),3),
           ci_low = round(exp(summary_ORtab_Intergen_ER[,1] - 1.96*summary_ORtab_Intergen_ER[,2]),3),
           ci_high = round(exp(summary_ORtab_Intergen_ER[,1] + 1.96*summary_ORtab_Intergen_ER[,2]),3),
           pval = scales::pvalue(summary_ORtab_Intergen_ER[,4]),
           row.names = NULL)
